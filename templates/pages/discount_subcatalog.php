...
<div class="ricon ri-<?=$catalogname?>"></div>
    <h2 id="h1-rubric"><a href="/discount/<?=$catalog['cat']?>/" title=""><?=$catalog['caption']?></a>&nbsp;<span class="count_items"><?=$catalog['discount_count']?></span></h2>
    ...
    <div class="sh">
		...
		<?foreach($subcats as $ssubcat):?>
			...
			?>
				<?if($ssubcat['discount_count']>0):?>
				<div class="<?=($subcat == $ssubcat['cat'] ? 'selected':'')?>"> <a href='/discount/<?=$catalog['cat']?>/<?=$ssubcat['cat']?>/'><?=$ssubcat['caption']?></a><span class="count_items">(<?=$ssubcat['discount_count']?>)</span>, </div>
				<?endif?>
		<?endforeach?>
		</div>
    </div>
...
<?if(strlen($catdescription)):?>
<div class="r-description">В разделе найдете: <em><strong><?=$catdescription?></strong></em></div>
<?endif?>
...
<div class=sresi>
	<div class="co">
		...
		Адрес: <strong><?=Core::join_strings($firm['addr']['region'].", ".$firm['addr']['district'].", ".$firm['addr']['street'].", ".$firm['addr']['dom'],",")?></strong> 
		<?endif?>
