...
/*			case 3:
	...
	memset(sendbuff, '\0', sizeof(sendbuff));
	snprintf(sendbuff, sizeof(sendbuff), "Found %d Subdomain(s) for host %s, Searched %d pages containing %d results\n", subcount, hostwww, totalpages, totalpages*100);
	print_line(sendbuff);
...
int sublist(char *sub, char *host)
{
	...
	subbuff = realloc(subbuff, subcount * (sizeof(char) * NCOL));

	if (!subbuff){
		print_line("ERROR: Realloc in SubSearch module failed\n");
		return 1;
	}
	subbuff[subcount] = malloc(NCOL*sizeof(char));
